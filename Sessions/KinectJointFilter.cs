...
namespace Sessions
{
    class KinectJointFilter
    {
        ...
        CameraSpacePoint[] m_pFilteredJoints;                 // Stores the filtered data. Filter output.          
        FilterDoubleExponentialData[] m_pHistory;             // Stores slided window of data points.
        ...
        public void UpdateFilter(Body pBody)
        {
            ...
            for (JointType jt = JointType.SpineBase; jt <= JointType.ThumbRight; jt++)
            {
                ...
                if(jt == JointType.AnkleLeft || jt == JointType.AnkleRight ||
                   jt == JointType.KneeLeft || jt == JointType.KneeRight)
                {
                    SmoothingParams.fSmoothing = 0.75f;
                } else if(jt == JointType.ShoulderLeft || jt == JointType.ShoulderRight ||
                    jt == JointType.SpineShoulder || jt == JointType.SpineBase || jt == JointType.Head)
                {
                    SmoothingParams.fSmoothing = 0.25f;
                    SmoothingParams.fJitterRadius *= 2.0f;
                }
        ...
        /// <summary>
        /// Returns filtered joints data. Output of the filter.
        /// </summary>
        /// <returns></returns>
        ...
        void UpdateJoint(Body body, JointType jt, TRANSFORM_SMOOTH_PARAMETERS smoothingParams)
        {
            ...
            else
            {
                ...
                if (fDiff <= smoothingParams.fJitterRadius)          // it was <= before.
                {
                ...
                // Now the double exponential smoothing filter (it was 1 - fsmoothing and fsmoothing)
                vFilteredPosition = CSVectorAdd(CSVectorScale(vFilteredPosition, smoothingParams.fSmoothing),
                    CSVectorScale(CSVectorAdd(vPrevFilteredPosition, vPrevTrend), 1.0f - smoothingParams.fSmoothing));
